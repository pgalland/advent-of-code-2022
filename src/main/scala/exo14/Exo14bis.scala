...
object Exo14bis {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val nbRows    = rocks.map(_._1).max + 1 + 2
      val shift     = nbRows
      val nbColumns = math.max(rocks.map(_._2).max + 1, 501) + 10 * shift
      val grid      = (1 to nbRows).map(_ => Array.fill(nbColumns)(false)).toArray
      grid(nbRows - 1) = Array.fill(nbColumns)(true) // the ground.
      rocks.foreach { case (x, y) => grid(x)(y + shift) = true }

      var nbGrainsResting = 0
      while (!grid(0)(shift + 500)) {
        dropGrainOfSand(position = 0 -> (shift + 500), grid)
        nbGrainsResting += 1
      }
  ...
  def printGrid(grid: Array[Array[Boolean]]) = {
    grid.foreach(line => println(line.map(x => if (x) "#" else ".").mkString))
    println("========================")
    println("========================")
  }
}
